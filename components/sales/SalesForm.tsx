...
export function SalesForm({
  ...
  const fetchNextReference = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          "Failed to fetch next sale reference, status:",
          response.status,
          await response.text(),
        );
        return "";
      ...
      if (payload && typeof (payload as any).reference === "string") {
        const nextReference = (payload as any).reference;
        setReference(nextReference);
        return nextReference;
      }

      console.warn("Unexpected reference response:", payload);
      return "";
    } catch (error) {
      console.warn("Failed to fetch next sale reference:", error);
      return "";
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const saleNumber = isEditing ? sale.saleNumber : reference;
      const newTxnId =
