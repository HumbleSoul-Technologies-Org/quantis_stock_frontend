  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
      {cards.map((card, index) => {
        ...
        return (
          <Card key={index} className={`${card.borderColor} border-2 bg-white dark:bg-slate-800`}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 p-3 sm:p-4">
              <CardTitle className="text-xs sm:text-sm font-medium text-gray-700 dark:text-slate-300">{card.title}</CardTitle>
              <div className={`${card.color} dark:bg-opacity-20 p-2 rounded-lg`}>
                <Icon className={`w-4 sm:w-5 h-4 sm:h-5 ${card.iconColor}`} />
              </div>
            </CardHeader>
            <CardContent className="p-3 sm:p-4">
              <div className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-teal-100 break-words">{card.value}</div>
            </CardContent>
          </Card>
        );
      })}
