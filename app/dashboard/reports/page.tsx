...
export default function ReportsPage() {
  ...
  const averageStockLevel = products.length > 0 ? Math.round(products.reduce((sum, p) => sum + p.currentStock, 0) / products.length) : 0;
  ...
  const exportCSV = () => {
    ...
    element.setAttribute('download', `${selectedReport}-report-${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-2">View insights and generate reports</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        <Button
          ...
          className={selectedReport === 'inventory' ? 'bg-green-600 hover:bg-green-700' : ''}
        >
          Inventory Report
        </Button>
        <Button
          ...
          className={selectedReport === 'sales' ? 'bg-green-600 hover:bg-green-700' : ''}
        >
          Sales Report
        </Button>
        <Button
          ...
          className={selectedReport === 'summary' ? 'bg-green-600 hover:bg-green-700' : ''}
        >
          Summary
        </Button>
      ...
      {selectedReport === 'summary' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Card className="border-green-200 border-2">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-gray-900">{formatCurrency(totalInventoryValue)}</div>
            </CardContent>
          </Card>

          <Card className="border-green-200 border-2">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-gray-700">Avg Stock Level</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-gray-900">{averageStockLevel}</div>
            </CardContent>
          </Card>
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                ...
                <p className="text-2xl font-bold text-purple-900 mt-1">{formatCurrency(totalInventoryValue)}</p>
              </div>
              <div className="p-4 bg-red-50 rounded-lg border border-red-200">
                <p className="text-sm text-red-600 font-medium">Low Stock Items</p>
                <p className="text-2xl font-bold text-red-900 mt-1">{lowStockItems.length}</p>
              </div>
            </div>

            <div>
              <h3 className="font-semibold text-gray-900 mb-3">Top 5 Best Selling Products</h3>
                ...
                <div className="space-y-2">
                  {topProducts.map((p, idx) => (
                    <div key={p.id} className="flex items-center justify-between p-3 bg-gray-50 rounded border border-gray-200">
                      ...
                      <p className="font-bold text-gray-900">{p.salesVolume} units sold</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                ...
                <p className="text-2xl font-bold text-green-900 mt-1">{completedSales.length}</p>
              </div>
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                ...
                <p className="text-2xl font-bold text-purple-900 mt-1">{formatCurrency(avgOrderValue)}</p>
              </div>
            </div>
                    ...
                    <tbody>
                      {completedSales.slice(0, 10).map((s) => (
                        <tr key={s.id} className="border-b border-gray-200 hover:bg-gray-50">
                          ...
                          <td className="p-3 text-center text-gray-600">{s.items.length}</td>
